...
class Ipfs {
  ...
  async uploadMetaData(imageFileName, cid, description) {
    const oThis = this;
  ...
  async uploadSeedWhisperMetaData(cid) {
    ...
    const imageLink = `ipfs://${cid}`;
    let result = null;
    const whisperOfTheDay = `Whisper of the day is out now! Generate your own mutation and add now on https://whisperchain.xyz/`;
    const postData = {
      version: '2.0.0',
      mainContentFocus: 'IMAGE',
      metadata_id: uuid(),
