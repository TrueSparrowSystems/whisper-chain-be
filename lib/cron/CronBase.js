...
class CronBase {
  ...
  constructor() {
    ...
    oThis.processLockFile = `/tmp/${oThis.constructor.name}.lock`;

    oThis._attachHandlers(); // Attaching handlers from sigint handler.

    new FileIo().createIfnotExists(oThis.processLockFile);

    try {
      oThis.releaseLockFunction = lockfile.lockSync(oThis.processLockFile);
    } catch (err) {
      process.emit('SIGINT');
    }
  }
