...
class SeedImage extends CronBase {
  ...
  async _validateWhitelistedUser() {
    ...
    return !userIds.includes(String(oThis.message.from.id));
  }
  ...
  async _downloadImage() {
    ...
    const localFilePath = `${oThis.imageDir}/${oThis.filePath}`;

    basicHelper.ensureDirectoryExistence(localFilePath);

    console.log('_downloadImage localFilePath --------------------', localFilePath);

    await oThis._downloadRemoteFile(downloadURL, localFilePath);
  }
