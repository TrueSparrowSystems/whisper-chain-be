...
class CookieHelper {
  ...
  /**
   * Parse lens user cookie for logout.
   *
   * @param {object} req
   * @param {object} res
   * @param {function} next
   ...
   */
  async parseLensUserLoginCookieForLogout(req, res, next) {
    let loginCookieValue = req.signedCookies[cookieConstants.lensUserLoginCookieName];

    cookieHelperObj.deleteUserLoginCookie(req, res, { cookieName: cookieConstants.lensUserLoginCookieName });
